...
export const errorCatalog: Record<string, ErrorInfo> = {
    ...
    [CodeTextTooLongError.name]: { statusCode: 400, message: 'Code text is too long' },
    ...
    [LanguageNotFoundError.name]: { statusCode: 404, message: 'Language not found' },
    [LanguageMissingNameError.name]: { statusCode: 400, message: 'Missing language name' },
    [UserNotFoundError.name]: { statusCode: 404, message: 'User not found' },
    [UserAuthIdTooLongError.name]: { statusCode: 400, message: 'Auth ID cannot be longer than 256 characters' },
    [UserGithubIdTooLongError.name]: { statusCode: 400, message: 'Github ID cannot be longer than 256 characters' },
    [UserGithubUsernameTooLongError.name]: { statusCode: 400, message: 'Github username cannot be longer than 256 characters' },
    [UserNameTooLongError.name]: { statusCode: 400, message: 'Name cannot be longer than 256 characters' },
    [UserImageTooLongError.name]: { statusCode: 400, message: 'Image cannot be longer than 256 characters' },
    [InvalidArgumentError.name]: { statusCode: 400, message: 'Invalid argument' },
};

export function getErrorInfo(error: Error): ErrorInfo {
    return errorCatalog[error.constructor.name] || { statusCode: 500, message: 'Internal Server Error' };
}
